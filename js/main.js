...
// Let all four hero messages play before the image plane begins to rise.
// rise slowly into place. Deliberate interaction before it begins hands control
// back to the visitor; once settled, the next gesture opens the next section.
let autoRevealTimer;
let autoRevealFrame;
let autoRevealCancelled = false;
let autoRevealPreviousScrollBehavior = "";
let autoRevealInputController;

if ("scrollRestoration" in history) history.scrollRestoration = "manual";

function cancelAutoReveal() {
  autoRevealCancelled = true;
  window.clearTimeout(autoRevealTimer);
  if (autoRevealFrame) window.cancelAnimationFrame(autoRevealFrame);
  document.documentElement.style.scrollBehavior = autoRevealPreviousScrollBehavior;
}
function bindAutoRevealCancellation() {
  autoRevealInputController?.abort();
  autoRevealInputController = new AbortController();
  const options = { once: true, passive: true, signal: autoRevealInputController.signal };
  ["wheel", "touchstart", "pointerdown", "keydown"].forEach((eventName) =>
    window.addEventListener(eventName, cancelAutoReveal, options),
  );
}
function runAutoReveal() {
  if (!revealSection || autoRevealCancelled || window.scrollY > 8) return;
  const startY = window.scrollY;
  const destinationY = Math.max(0, revealSection.offsetTop);
  const distance = destinationY - startY;
  const duration = 3200;
  const startTime = performance.now();
  autoRevealPreviousScrollBehavior = document.documentElement.style.scrollBehavior;
  document.documentElement.style.scrollBehavior = "auto";
  document.body.classList.add("is-auto-revealing");
  function step(now) {
    if (autoRevealCancelled) {
      document.body.classList.remove("is-auto-revealing");
      document.documentElement.style.scrollBehavior = autoRevealPreviousScrollBehavior;
      return;
    }
    const progress = clamp01((now - startTime) / duration);
    // Quintic smoothstep: no abrupt acceleration at the start or stop.
    const eased = progress * progress * progress * (progress * (progress * 6 - 15) + 10);
    window.scrollTo(0, startY + distance * eased);
    if (progress < 1) {
      autoRevealFrame = window.requestAnimationFrame(step);
    } else {
      document.body.classList.remove("is-auto-revealing");
      document.body.classList.add("has-auto-revealed");
      document.documentElement.style.scrollBehavior = autoRevealPreviousScrollBehavior;
    }
  }
  autoRevealFrame = window.requestAnimationFrame(step);
...
function armAutoReveal() {
  ...
  window.clearTimeout(autoRevealTimer);
  if (autoRevealFrame) window.cancelAnimationFrame(autoRevealFrame);
  autoRevealCancelled = false;
  autoRevealFrame = null;
  document.body.classList.remove("is-auto-revealing", "has-auto-revealed");
  window.scrollTo({ top: 0, left: 0, behavior: "auto" });

  bindAutoRevealCancellation();

  autoRevealTimer = window.setTimeout(runAutoReveal, 16000);
}

function restartHomeSequence() {
  autoRevealInputController?.abort();
  window.clearTimeout(autoRevealTimer);
  if (autoRevealFrame) window.cancelAnimationFrame(autoRevealFrame);
  autoRevealCancelled = false;
  document.documentElement.style.scrollBehavior = "auto";
  window.scrollTo({ top: 0, left: 0, behavior: "auto" });
  window.requestAnimationFrame(() => window.setTimeout(armAutoReveal, 40));
}
...
function openNextSection(event) {
  if (!statementSection || nextSectionTransitioning || !imageIsSettled()) return false;
...
let deliveryIndex = 0;
let deliveryTimer;
let deliveryHighlight;
function positionDeliveryHighlight(index, animateText = true) {
  if (!deliveryGrid || !deliveryCards.length) return;
  const card = deliveryCards[index];
  const gridRect = deliveryGrid.getBoundingClientRect();
  const cardRect = card.getBoundingClientRect();
  const isFirstEntry = index === 0;
  if (isFirstEntry) {
    deliveryHighlight.classList.add("is-first-entry");
    deliveryHighlight.style.width = "0px";
    deliveryHighlight.style.height = `${cardRect.height}px`;
    deliveryHighlight.style.transform = "translate(0px, 0px)";
    deliveryHighlight.getBoundingClientRect();
  } else {
    deliveryHighlight.classList.remove("is-first-entry");
  }
  deliveryHighlight.style.width = `${cardRect.width}px`;
  deliveryHighlight.style.height = `${cardRect.height}px`;
  deliveryHighlight.style.transform = `translate(${cardRect.left - gridRect.left}px, ${cardRect.top - gridRect.top}px)`;
  ...
  deliveryHighlight.dataset.ready = "true";
}
if (deliveryCards.length) {
  deliveryHighlight = document.createElement("span");
  deliveryHighlight.className = "delivery-highlight";
  deliveryHighlight.setAttribute("aria-hidden", "true");
  deliveryGrid.prepend(deliveryHighlight);
  positionDeliveryHighlight(0, false);
  if (!window.matchMedia("(prefers-reduced-motion: reduce)").matches) {
    deliveryTimer = window.setInterval(() => {
      deliveryIndex = (deliveryIndex + 1) % deliveryCards.length;
      positionDeliveryHighlight(deliveryIndex);
    }, 2800);
  }
  window.addEventListener("resize", () => positionDeliveryHighlight(deliveryIndex, false));
}
...
function updateHeaderTheme() {
  ...
  const onDark = menuOpen || Boolean(point && point.closest(".reveal, .statement, .industries, .contact-section, .footer"));
...
document.querySelector("form").addEventListener("submit", (e) => {
  ...
  e.currentTarget.querySelector(".form-note").textContent =
    "Thank you. This review form is ready to connect to your preferred email or backend service.";
});
